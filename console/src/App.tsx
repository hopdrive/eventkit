    ...
    <div className='h-screen flex bg-gray-50 dark:bg-gray-900'>
        ...
        <div className='flex flex-col h-full'>
          ...
          <div className='flex items-center h-16 px-4 border-b border-gray-200 dark:border-gray-700'>
            <div className='flex items-center'>
              ...
              <span className='ml-3 text-lg font-semibold text-gray-900 dark:text-white'>EventKit Console</span>
            </div>
          </div>
      ...
      <div className='flex-1 flex flex-col overflow-hidden'>
        ...
        <main className='flex-1 overflow-auto bg-gray-50 dark:bg-gray-900'>
          {/* Route-transition animation removed: the enter tween stalled mid-flight on
              canvas-heavy pages (ReactFlow mount + autoFocus URL replace), leaving the
              whole page frozen at fractional opacity. A plain container is also snappier
              per navigation. */}
...
function App() {
  ...
  const [timeRange, setTimeRange] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('eventkit-console-timeRange');
      return saved || '24h';
    }
    return '24h';
  });
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('eventkit-console-timeRange', timeRange);
    }
  }, [timeRange]);
