...
**Result-driven response: `respond` (ADR-029, amends ADR-026, v0.3.16).** `resolve` is sibling-ignorant on purpose. It runs concurrently with the jobs and never reads their results, which is exactly right for a fast ack that only depends on the request. But sometimes the reply has to reflect how the work actually went: run N jobs, then answer based on the combined result. You used to have to eject from `kit.handler()` and hand-roll `kit.handle()` for that. Now a module can declare `respond(ctx, { jobs, ok }) => output` instead of `resolve`. The runtime runs the module's `jobs` under its `run` config, waits for them, then calls `respond` with the settled `JobExecution[]` and an `ok` flag (`every job completed|skipped`, the same rule as `InvocationResult.ok`). The return becomes `InvocationResult.resolved.output`, and a thrown `ClientError`/`ActionError` becomes `resolved.error`. That's the exact same downstream plumbing as `resolve`, so every platform maps it for free. Which seam you declare is the config (declarative, like ADR-025), so the response timing stays easy to read off the module. `resolve` and `respond` are mutually exclusive (register-time error). `respond` needs at least one job, since it reads results. And a `respond` module is rejected at `validate()` under a `deferredResponse` platform (background/202, where the response is already gone). Fire-and-forget stays the default: declare neither. Jobs keep their own retry and durability. `respond` only reads their executions to shape the reply. New core types are `RespondFunction`/`JobsResult`, plus a `PlatformAdapter.deferredResponse` flag (set on `netlifyBackgroundPlatform`).
...
- **Ack semantics flip — choose `respond` or `resolve` per endpoint, explicitly.** SD/CD/CA return **500 on business-logic failure today**, which is what drives the vendor to retry. EventKit's fire-and-forget webhook default acks **200 regardless of job outcome**. Porting one of these endpoints without deliberately picking `respond` (job-failure → 5xx) silently turns "the vendor retries our failures" into "we ack and swallow them." The migration checklist MUST record the ack choice for every webhook endpoint. (Validation of the design: super-dispatch's handler is accidentally a *background* function today, so its computed 401/500 responses are already dead code — EventKit's `deferredResponse` guardrail rejects a `respond` module on a background platform at register time, which would have caught that.)
