...
| 2026-07-01 | 0.3.18 (draft) | Rob Newton + Claude | **Series job execution deferred, parallel-only at launch (ADR-031, amends ADR-014).** The `run.mode` (`'series'`) switch and `continueOnFailure` (module- and per-job) are specified but not enabled. The public `RunOptions`/`JobOptions` leave them out, and the runtime runs every module's jobs in parallel with isolated failures. Held back because series invites the sequential inter-job coupling ADR-025 removed, with more abuse risk than value right now. Kept as a documented possible future feature behind the same `run.mode` API. Updated §9.5, §9.4, ADR-014, the §0 summary, and the `'skipped'` status note. |
| 2026-06-30 | 0.3.17 (draft) | Rob Newton + Claude | **Webhook `rejectUnverified`, one-chokepoint signature rejection (ADR-030).** With `webhook({ vendor, verify, rejectUnverified })`, a failed or throwing `verify` gets rejected with **401** (configurable via `{ status, message }`) before detection, so no detector needs a `signatureVerified` guard. Mechanism: the adapter throws `ClientError` from `normalize`, and the runtime now maps a pre-dispatch `ClientError` (duck-typed `.status`) to that wire status via `resolved.error` instead of the framework 500, skipping dispatch. Requires `verify`. Trade-off: a rejected request leaves no Invocation record, just a framework `warn`. Keep the default and guard in the detector when the forged attempt must be recorded. Updated §7.1. |
| 2026-07-02 | 0.3.22 (draft) | Rob Newton + Claude | **Per-source crash policy (ADR-038).** The error-path chaos matrix (ADR-036) surfaced that a **detector**/**`prepare`** crash lands in `events[].error` with no top-level `result.error`, so `httpResponse` returns **200** and an at-least-once sender never retries — right for Hasura (poison-row protection, legacy parity), wrong for the reverse-integration webhooks (a transient crash silently drops a redeliverable vendor event). Added `EventKitPlugin.crashPolicy: 'ack' \| 'signalRetry'` declared by the source (default `'ack'`). `'signalRetry'` escalates a processing crash to `result.error` → **500** + a loud `invocation.log.error`; a deliberate `resolve`/`respond` reply or a job failure is not a crash and is never escalated. The **webhook source defaults to `'signalRetry'`** (overridable via `webhook({ crashPolicy })`); Hasura/application stay `'ack'`. Updated §7.1, §22, D36. |
| 2026-07-03 | 0.3.24 (draft) | Rob Newton + Claude | **Selective replay + declarative job-module config (ADR-042/043).** **ADR-042**: operators rerun CHOSEN jobs of a recorded invocation via `x-eventkit-replay-of`/`-jobs` headers on a byte-identical re-POST; unselected jobs finish `'skipped'` (reason `rerun job not selected`) with both job hooks emitted — the runtime's first producer of the reserved `'skipped'` status (`ctx.skip()` stays `'completed'`+conditionNotMet; the pair is API). Filter rides `request.meta[JOB_FILTER_KEY]` via the new opt-in `replayControls()` plugin (malformed directive → branded ClientError 400); selection only narrows — auth/verification/suppress-dispatch never bypassed. `JobOptions.replaySafe?: boolean` (tri-state) declares rerun safety, persisted to `job_options` + reported by `describe()` for the Console's warning ladder. Prerequisite: widen the legacy observability job-status CHECK (sink default maps `skipped→failed`). Design: `selective-replay-spec.md`. **ADR-043**: `JobOptions` is the ONE job-settings interface, now also writable declaratively on the job function as `fn.config` (`defineJob(fn, config)` sugar); `job(fn, opts)` merges per-property, call-site wins, module config fills the rest. Added D40–D41. |
| 2026-07-02 | 0.3.23 (draft) | Rob Newton + Claude | **Chain-concern placement + correlation-id shape + loop elevation (ADR-039/040/041).** A live E2E proof (session-variable header channel + vendor round trip via `correlationResolver`; consumer writeup `docs/chained-events.md`) surfaced three structural findings, ratified after a placement audit. **ADR-039**: the tracking-token codec moves to **core**; **sources own inbound token discovery** (the Hasura family surfaces `meta.tokenCandidates` from the write field + session variables; the generic `loopGuard` sheds all Hasura payload anatomy and consumes candidates); outbound header injection gets its paved road via the **ADR-020 input-baseline seam** (`ctx.input.gql` from a Hasura companion helper) while the durable answer stays the sdk-* write layer; observability remains a pure lineage consumer. **ADR-040**: a correlation id is **UUID or 32-hex dashless** — Hasura `trace_context.trace_id` roots are dashless, and UUID-only validation silently broke every token parse in the proof; trace-id adoption stays (APM linkage). **ADR-041**: a hop-ceiling halt is a **first-class loud event** — branded `LoopDetectedError` through `onError` (Sentry-routable), a durable halted marker on the invocation record (queryable), `warnAtDepth` escalates through `onError` as non-fatal, HTTP stays 200 (never invite a retry of a loop). Updated §22; added D37–D39. |
| 2026-07-01 | 0.3.21 (draft) | Rob Newton + Claude | **Test-coverage strategy (ADR-036/037 + §20).** Folded a Fable-5 test-coverage review. Added `docs/planning/testing-strategy.md` (the full plan). §20 reframed: the suite is the executable form of the spec — every normative MUST maps to a named test. Headline package tests: an **error-path chaos matrix** (throw at every seam, assert five invariants, ADR-033), **golden-trace observability snapshots** (record schema = Grafana/Console contract), a **`formatResponse` matrix** (outcome × platform), a **docs-compile CI gate** (kills the doc-drift defect class), lifecycle-ordering snapshot, concurrency/warm-instance soak, scoped property tests, and API-snapshot + nightly mutation. **ADR-036**: `@hopdrive/eventkit/testing` is a versioned public harness (payload builders, `testInvocation`/`recordingPlugin`, `detectorContract` with auto-MANUAL, memory doubles, `simulateChain`, flow assertions) + the standard consumer test pyramid. **ADR-037**: flow artifact gains `toFlowMermaid`, `eventkit-flow coverage`/`simulate`, and reserves `repo`/`function` + job `metadata.effects` schema now for a deferred cross-kit aggregator; proto-Compare CI helper validates the D9 matcher on one flow early. Updated §17, §20, §22, D34/D35. |
| 2026-07-01 | 0.3.20 (draft) | Rob Newton + Claude | **Principal-review hardening pass (ADR-033/034/035 + P0 fixes).** Folded a Fable-5 architect review. **ADR-033** pre-dispatch pipeline hardening: isolate `configureInvocation`/`augmentEnvelope` (throw → `onError`, continue), brand-check the pre-dispatch `ClientError` fast-path (any other error → framework 500, never silent `ok:true`), move `onInvocationEnd`/`onFlush` into `finally`, deep-merge `augmentEnvelope` `meta` (fixes P0-1/P0-2/P0-3). **ADR-034** optional `loopGuard` hop-depth ceiling (`warnAtDepth`/`haltAtDepth`, off by default). **ADR-035** `ctx.skip(reason)` job self-skip recording `condition_not_met` (§9.4/§14/§19.1). P0-4: §12.4 clarified — a row with a live durable retry MUST read `delaying`/`ready`, not terminal `error` (code drift). P0-5: no spec fix (codec dot default and §13 agree; pipe is the legacy preset) — instead the shared HopDrive `loopGuard` preset is now *required* during migration + D6 asserts the token byte-for-byte (§13). Doc-drift fixes: `augmentJobContext` returns `{ input?, ambient? }` (was `context?`); `observability`/`batch` registration examples use `{ sink: graphqlSink(...) }` / `{ store }` (was `{ graphql }`). Typed `prepare`→job-`input` generic (§18). `verify` receives body+headers+query + HopDrive verify presets (§7.1). Reverse-integration migration hazards (§19.3). Playbook count corrected to ~21 (§19.1). |
...
| 2026-07-09 | 0.3.25 (draft) | Rob Newton + Claude | **The HTTP reply moved to the invocation layer (ADR-044, re-amends ADR-026, supersedes ADR-029's `respond`).** A module owns only a detector + a static (now effectively required) `jobs` array — declaring `resolve`/`respond`/`response` throws at register time. The endpoint's one reply is declared at `kit.handler({ before, after })`: `before` is a pre-dispatch gate on the raw args (auth/method, before `normalize`), `after` is the reply applied once the run settles — `{ body }` (a constant ack) or `{ fromResults: (result) => body }` composing over the full typed `InvocationResult` rollup (every `EventOutcome`/`JobExecution`). "202 now, then work" stays a platform choice (`deferredResponse`/background rejects `{ fromResults }` at handler creation, ignores the `{ body }` value); `after` is skipped on a framework 500 and never clobbers a pre-dispatch client rejection. Removed `ResolveFunction`/`RespondFunction`/`FlowResponseKind`/`KitEventDescription.response`, the response-kind in `describe()`/flow output, and `expectFlow(...).respondsWith(...)`. Updated §2.1, §3.5, §5, §6, §7.1, §7.2, §8, §9.7, §18, §19.2, §19.3, §20, and ADR-026. |
...
- **[D] Parallel, isolated job execution** (§9, §10). Every module runs its jobs in parallel with isolated failures, matching the current `Promise.allSettled` fan-out. **[v0.3.18, ADR-031] The configurable `mode: 'series'` switch (plus `continueOnFailure`) is deferred, not enabled in the initial release.** It stays specified as a possible future control (§9.5).
...
- EventKit MUST preserve the event-module authoring model: business-event dot-notation names; each module declares exactly one detector and a static `jobs` set. There is no handler body — the module is declarative (ADR-025) — and the HTTP reply is not a module concern: it is declared at the invocation layer (`kit.handler({ after })`, ADR-044).
...
Each module MUST declare exactly one detector. The work it triggers is a **static `jobs` array** — there is no handler function (ADR-025). The invocation's HTTP reply is declared separately at `kit.handler({ after })` (ADR-044), never on the module.
...
Every inbound request enters through a Source Adapter, which constructs an EventEnvelope and a DetectorContext. The runtime evaluates every registered detector independently; positive detections produce DetectedEvents whose declared `jobs` the **runtime** executes (after the module's optional `prepare`); the invocation's HTTP reply is declared at the invocation layer (`kit.handler({ after })`) and produced once the run settles (ADR-025/026/044). Plugins surround the runtime without owning it.

The runtime guarantees deterministic ordering: normalization → detection → prepare → jobs, with consistent plugin callback ordering; the invocation-layer `after` reply (`kit.handler`) is applied once the run settles. Errors are isolated: a detector failure affects only that detector; a job failure is isolated from other jobs by default (§10, ADR-014).
...
7. **Reply** — **[v0.3.11; re-amended ADR-044]** the invocation's HTTP reply is declared at `kit.handler({ after })`, not on any module. Once the run settles, the runtime produces the reply from `after` — a constant `{ body }` body, or `{ fromResults: (result) => body }` composing over the full `InvocationResult` (every detected event's outcome and its job executions) — and surfaces it on `InvocationResult.resolved`; a thrown `ClientError`/`ActionError` is captured there too.
...
**Signature verification placement (and `rejectUnverified`, ADR-030, v0.3.17).** `verify` runs **once** per request, in `normalize`, before any module. By default it never throws. It sets `ctx.signatureVerified` and lets the detector decide whether to fire (§7.1). That keeps forged attempts observable and lets per-event policy differ. So the detector's `signatureVerified && …` is a cheap boolean read, not a re-verification. `handler({ before })` is not the place for signatures. It runs before `normalize` on the raw args, so it can't see `signatureVerified` without redoing the HMAC. Keep it for cheap request-level gates like the method or an auth-header check. For an endpoint where every event needs a valid signature, set `rejectUnverified: true` (or `{ status?, message? }`). A failed or throwing `verify` then gets rejected with **401** before detection: the adapter throws `ClientError`, the runtime maps a pre-dispatch `ClientError` to that wire status via `resolved.error`, and dispatch is skipped. So no detector guard is needed. It requires `verify`. Trade-off: a request rejected at `normalize` never becomes an event, so there's **no Invocation/Event/Job record**, just a framework `warn`. Keep the default and guard in the detector when you need the forged attempt recorded for telemetry.

**[v0.3.20] What `verify` receives, and shipped presets.** `verify` MUST receive everything a real vendor scheme needs: the **parsed body**, the **headers**, and the **query params**. Super-dispatch keys its token check on a field in the *body*; the vendor query-param plumbing is the still-open item in `external-correlation-chaining.md` §8. The `verify` signature MUST expose all three so no scheme has to reach around the adapter. And EventKit SHOULD ship **HopDrive verify presets** so each repo stops hand-rolling the same four schemes: HMAC (`t=`/`v1=`, Stripe-style), static-header token, shared-secret, and Hasura passphrase. Verification is a real security gap today, not just a consistency one — Twilio, Dealerware, plateau, both hoprides vendor webhooks, and the inter-repo `hopdrive-event` webhooks currently have **no** signature verification. Presets + `rejectUnverified` close that.

**[v0.3.22] Crash policy defaults to `'signalRetry'` for webhooks (ADR-038).** Unlike Hasura, a vendor webhook should get a **500 and retry** when a detector or `prepare` *crashes* (a transient failure would otherwise be silently dropped, since the crash lands in `events[].error` with no top-level `result.error`). So the webhook source sets `crashPolicy: 'signalRetry'` by default: an unhandled processing crash escalates to `result.error` → 500 + a loud `invocation.log.error`, and the sender redelivers. Override with `webhook({ crashPolicy: 'ack' })` for an endpoint that must not retry. A deliberate `after` reply (including a thrown `ClientError`/`ActionError` from `fromResults`) is **not** a crash and keeps its own wire status; a job failure keeps its own durable retry and stays 200. See ADR-038 (§22) for the full contract.
...
**`hasuraAction` source adapter** (`sourceType: 'action'`). `normalize()` wraps the payload into an envelope; detector/handler context exposes: `actionName`, `input` (typed args), `sessionVariables` (`role`/`userId`/`email`), and `requestQuery?`. A detector typically matches `ctx.actionName`. Authoring helpers `hasuraAction.detector`/`hasuraAction.prepare`, plus the source-scoped `hasuraAction.defineEvent<Input>({…})`.

**How an action returns a synchronous response (ADR-026, re-amended by ADR-044).** EventKit modules are declarative *fire-and-forget* job sets (ADR-025); an action must return a value, but that value is **not** a module concern — one invocation has one wire reply and it lives at the invocation layer. The action's work runs as durable **`jobs`**, and the endpoint declares its reply at **`kit.handler({ after: { fromResults: (result) => output } })`**, composing the response body from the full `InvocationResult` (every detected event's outcome + its job executions). The **generic** platform adapter the function already uses (`netlifyV2Platform` / `netlifyPlatform` / `lambdaPlatform`) maps that return → the 2xx body, and a thrown `ClientError`/`ActionError(message, code?)` → the 4xx `{ message, extensions }` shape — there is **no** dedicated action platform (the response contract composes with transport; v0.3.11, below). Rationale: keeps the reply explicit at the one layer that owns it, composed from the run rather than overloading a job's output as the response. The `after` reply is **source-agnostic** — any source's endpoint may declare it (a Stripe `webhook` shaping its required response, a future RPC source) — and the source's platform adapter maps the result to the wire. The win that makes an action worth running through EventKit at all: its downstream effects (jobs + DB writes that fire further events) are **correlation-traced back to the action** by the Observability plugin. The bespoke `app-*` endpoints are *replaced* by actions over time, not migrated.

**Two `after` modes — a constant ack vs a reply composed from the run (ADR-044, re-amends ADR-026, supersedes ADR-029's `respond`).** The reply at `kit.handler({ after })` is exactly one of two self-naming, mutually-exclusive modes. `{ body }` is a **constant** reply — data, not code, so the work provably cannot wait on or change it (the fast ack, e.g. Stripe's constant 2xx receipt); job failures stay Batch/observability's concern. `{ fromResults: (result) => body }` runs **arbitrary business logic over the full typed `InvocationResult`** the runtime builds — every detector's verdict as an `EventOutcome { name, detected, jobs, error? }` and every job as a `JobExecution { jobName, status, output, error, … }`, spanning all detected events on the request — after the run settles; the return becomes `InvocationResult.resolved.output`, and a thrown `ClientError`/`ActionError` becomes `resolved.error`. Both modes ride optional `status`/`headers` (`ResponseWire` — the web-standard `ResponseInit` fields, as data, beside the mode). "202 now, then run the work" is **not** an `after` mode — it is a **platform** choice (`netlifyBackgroundPlatform` / any `deferredResponse` platform); under it `{ fromResults }` is rejected (loudly, at handler creation — its value could never reach a reply that already left) and `{ body }` is allowed but its body is ignored. The whole `after` declaration is validated at handler **creation** (module scope), not on the first request; it is skipped on a framework error (a 500 always reaches the wire — the retry contract) and never clobbers a pre-dispatch client rejection (a webhook `rejectUnverified` 401 keeps its status).

**Two failure types — `ClientError` vs `ActionError` (why both).** An `after: { fromResults }` fn (or a job) reports failure by throwing one of two error classes, kept separate because the two callers react to fundamentally different things. `ClientError(status, message)` carries an **HTTP status**; the platform responds with that exact status — for a status-contract webhook (Stripe) the *status code* is the contract (it decides whether the vendor retries). `ActionError(message, code?, extensions?)` carries a **GraphQL error `code`**, *not* a status; the platform responds HTTP 4xx + `{ message, extensions: { code? } }`, which Hasura turns into a GraphQL `errors[]` entry — the GraphQL client reads `message`/`extensions.code` and never sees the HTTP status, so there is nothing meaningful to choose there. They are two different wire contracts (HTTP-status world vs. GraphQL-error world); folding them into one type would force a webhook author to invent a `code` and an action author to invent a `status` the client never sees. Both are read **duck-typed** by the runtime (not `instanceof` — unreliable across bundled module copies) and surfaced on `InvocationResult.resolved.error` for the generic platform to map.

# 8. Detector API and Event Module Authoring

The detector is the most important day-to-day API. It is a business-rule predicate answering one question: did this business event occur for this invocation? Every module MUST export exactly one detector, appearing at the top of the module (above its `jobs`).
...
export interface JobContext<TInput = undefined, TPayload = unknown, TMeta = Record<string, unknown>> {
  ...
  skip(reason: string): never;         // [v0.3.20, ADR-035] the job decided it had no work; records condition_not_met
  signal?: AbortSignal;
}
```

**[v0.3.20, ADR-035] `ctx.skip(reason)` — a first-class "no work to do" signal.** The Pattern-B short-circuit (§19.1) — `const { driverId } = ctx.input; if (!driverId) return;` — runs the job and completes normally, so its record looks identical to a job that did real work. That hides the branch every time a condition isn't met, which is precisely the visibility ADR-025's strictness was supposed to buy back, and it's the data Compare Mode's `condition_not_met` classification (§14) needs but nothing produces. `ctx.skip(reason)` fixes that: calling it stops the job (it throws a runtime-recognized `SkipSignal` the executor catches) and records the outcome as a branch-not-taken with the reason. It is **not** a new terminal status — the job still ends `'completed'` (it ran, it chose to do nothing) — the skip is recorded as structured metadata on the `JobExecution` (`metadata.conditionNotMet = { reason }`), so Observability and Compare Mode can render it distinctly without colliding with the reserved `'skipped'` status (which stays for the deferred series short-circuit, ADR-031). So the Pattern-B body becomes `if (!driverId) return ctx.skip('no driver on this outcome');`. The ~21 conditional sites (see §19.1) migrate without losing branch visibility.
...
export interface InvocationResult {
  ...
  timedOut?: boolean;
  error?: SerializedError;
  // [v0.3.11, ADR-026; re-amended ADR-044] The invocation's produced reply — from
  // `kit.handler({ after })` (the `{ body }` body or `{ fromResults }`'s return) or a
  // pre-dispatch client rejection (a source-thrown `ClientError`, e.g. webhook `rejectUnverified`).
...
**Shape 2 — Delta transform (bare verb, returns a partial; runtime merges).** "Contribute to a value the runtime is assembling." The runtime computes a base, then applies each plugin's returned partial in registration order with documented precedence. The default always contributes; plugins only add — so this shape is *inherently* "default + extra," never replacement. Members: `configureInvocation` (→ `Partial<RequestContext>`), `augmentEnvelope` (→ envelope refinements after the source builds it), `augmentJobContext` (→ `{ input?, ambient? }`). Per ADR-020, handler-supplied `options.input` wins over plugin `input` baselines. **[v0.3.20] `augmentEnvelope` deltas are deep-merged at the `meta` level** (`meta: { ...merged.meta, ...partial.meta }`), not shallow-overwritten — a plugin that returns `{ meta: { myKey } }` MUST NOT wipe another plugin's `sourceTrackingToken`/`sourceJobId` (ADR-033).
...
```ts
kit.use(observability, { sink: graphqlSink({ endpoint, adminSecret }) });   // factory + config
...
- **[v0.3.20] A row with a live retry MUST NOT read as terminally failed (ADR-033 / P0-4).** When `durableRetry` schedules a follow-up attempt (attempts remain), the observable state MUST be a non-terminal retry state (`'delaying'` → `'ready'`), matching the table above and legacy `@hopdrive/batch` watch-view semantics. Terminal `'error'` is reserved for the exhausted case (no further retry scheduled). The current implementation stamps the triggering row `'error'` while inserting the delayed retry row — that is a code-vs-spec drift on the AP/AR money path: an operator (or the live `batch_jobs` view) cannot tell "failed for good" from "retrying." The fix aligns the code to this contract; whether the same row is re-driven or a `delay_key`-deduped follow-up row carries the next attempt is an implementation choice, but a pending retry MUST be observably distinct from a dead job.
...
**[v0.3.20] Loop prevention today is loop *traceability*, not a cycle-breaker (ADR-034).** The token makes a self-triggered write recognizable, but nothing bounds a cycle: an A→B→A loop where a detector forgets its `sourceTrackingToken` guard runs forever, and the token only makes it visible after the fact. Since event storms are the stated catastrophic failure mode (ADR-016), `loopGuard` gains an **optional hop counter**: the token (or `meta`) carries a depth that increments each hop, and the plugin config takes a ceiling — `warnAtDepth` (log + `onError` breadcrumb) and `haltAtDepth` (hard-stop: suppress dispatch and log). Off by default (depth unbounded, today's behavior); when set it converts "hope every detector remembered the guard" into a bounded blast radius. Generic and config-driven, so it fits ADR-024.

**[v0.3.23] The suppress seam is structured, and a halt is loud (ADR-039/041).** Inbound discovery moved to the sources (`meta.tokenCandidates`, ADR-039.2) and `loopGuard` consumes candidates generically — the ADR-034 hop counter and ceilings are unchanged. The suppress seam's contract is now typed in core (`core/chain-guard.ts`): `envelope.meta.suppressDispatch` accepts a bare reason string OR a structured `SuppressDispatch { reason, error }`, and the runtime reports the (typically branded `LoopDetectedError`) through `onError` with phase `'chain-guard'` — never string-parsing the reason. A sibling `meta.chainGuardWarning = { error }` seam reports a NON-FATAL branded error (`ErrorContext.severity: 'warn'`) while dispatch proceeds — the `warnAtDepth` early alarm. The seam is runtime-reported because a plugin's `augmentEnvelope` cannot reach `onError` without throwing, and a throw is the wrong signal for a deliberate halt. Observability stamps the durable halted marker (`error_message` + `context_data.halted = { depth, ceiling }`); the wire response stays 200.

**[v0.3.20] The wire format is a consumer-misconfiguration hazard, not a package defect (P0-5, D23).** Core's codec defaults to a dot separator and §13's `${source}.${correlationId}.${ctx.job.id}` agrees with it — there's no drift. The legacy HopDrive extractor uses **pipe** (`source|correlationId|jobId`), available purely as a configurable preset (`codec: { separator: '|', validateCorrelationId: true }`). But because the separator is config, a HopDrive repo that registers `loopGuard` *without* the preset silently mints dot-tokens the legacy extractor can't parse during the dual-runtime window — chains sever and echo suppression goes dark with **no error anywhere**. Therefore, during migration a HopDrive repo MUST register `loopGuard` via the **shared HopDrive preset object** (pins `separator: '|'`, `validateCorrelationId`, `field: 'updated_by'`, and the service id) — a one-object import so the correct wire format is the path of least resistance, not a per-repo literal someone can typo. *(Shipped in v0.3.23 as `hopdriveLoopGuard({ serviceId, ... })`: it pins the codec and requires `serviceId`; the write field and session-variable name became SOURCE config with ADR-039.2, where the Hasura defaults — `updated_by`, `x-hasura-tracking-token` — already match, so the preset pairs with a default-configured Hasura source.)* This is the D23 preset, now required rather than optional for chain-participating functions. Paired with D6 shadow-mode asserting the stamped `updated_by` **byte-for-byte** across both runtimes before cutover, the window is covered at both ends: prevention at registration, detection at cutover.
...
**[v0.3.21] Planned surface growth (ADR-036/037; specced in `testing-strategy.md`).** `./testing` grows from today's `fakeSource`/`buildDetectorContextFor`/`defineFakeEvent`/name-validator into the standard consumer harness: per-source **payload builders** (`hasuraInsert`/`hasuraUpdate`/`hasuraManualEdit`/`hasuraCronPayload`/`hasuraActionPayload`, `webhookRequest().signWith()`), a **recording harness** (`testInvocation(kit, payload)` / `recordingPlugin()`), a **`detectorContract()`** helper (auto-appends a MANUAL case for `hasuraEvent` modules), **memory doubles** (`memorySink`/`memoryBatchStore`/`capturedLogger`), a **`simulateChain()`** correlation harness (`echoWebhook`/`lookupWebhook` + miss case), and **flow assertions** (`expectFlow(kit).event(…).hasJobs(…)`). `./flow` gains `toFlowMermaid`, an `eventkit-flow coverage` subcommand (every event has a detector test), an `eventkit-flow simulate --payload` (run pure detectors against a fixture), and the manifest schema reserves **`repo`/`function`** fields plus a job **`metadata.effects`** convention (`{ type:'db-write', table } | { type:'api-call', vendor }`) so a future org-level aggregator can infer cross-kit edges losslessly (ADR-037). `./testing` is held to the same API-snapshot discipline as the runtime.
...
kit.use(observability, { sink: graphqlSink({ endpoint: cfg.graphqlUrl, adminSecret: cfg.graphqlSecret }) });
kit.use(batch,     { store: batchStore });   // HopDrive wires a GraphQL-backed BatchJobStore (§12.1); durableRetry is opt-in
kit.use(loopGuard, hopdriveLoopGuard);        // shared HopDrive preset — pins separator '|', validateCorrelationId, field: 'updated_by', serviceId (§13)
...
// A module MUST declare `jobs` (the reply is invocation-level — `kit.handler({ after })`, ADR-044). A do-nothing module is a register-time error.
```
...
**[v0.3.20] `prepare`'s return type MUST flow into the jobs' `ctx.input` (typed prepare→input).** `prepare(ctx)` runs once and is merged into every job's `input` at runtime (ADR-025), but `defineEvent` is currently generic only over `TPayload`/`TMeta` — `jobs` are typed `any` on their input, so an author hand-types `ctx.input` in each job and keeps it in sync manually. This is the busiest seam in every module. `defineEvent` MUST thread `prepare`'s inferred return (`TPrepared`) through as the baseline of each job's `TInput`, so a job body sees `ctx.input` typed as the `prepare` output (plus its own per-job `input`) with no restatement. It is the single biggest day-to-day type-safety win for the ~245 modules and is a pure typing change — the runtime merge is already correct.
...
The declarative `jobs` array (ADR-025) makes conditional inclusion inexpressible, so the conditional-job sites must each move to one of two homes. **[v0.3.20] The consumer audit corrected the count to ~21 (not 14)**, and it surfaced a harder class the original rubric glossed: some conditions gate a job on data fetched *mid-handler* (a GraphQL query, a vendor API read) — `invoice.closed`, `outcome.resolving`, the super-dispatch offer-auto-accept. Those can't move to the detector without an async detector (discouraged, §8), so their realistic home is Pattern B, the job-internal short-circuit — which is exactly why `ctx.skip()` (§9.4, ADR-035) matters: it keeps that branch visible. The decision rubric is a single question: **does the condition decide *which business event occurred*, or merely *whether this one job has work to do*?**
...
**Pattern B — job-internal short-circuit (condition = data presence).** When the gate just means "this job has nothing to do for this row," keep the job in the static array and short-circuit at the top of the job body on its own input. Example — `db-outcomes/outcome.resolving` today does `if (driverId) jobs.push(job(notifyDriverOnResolution, …))`. Migrate to an unconditional `job(notifyDriverOnResolution, { input: ctx => ({ driverId: ctx.newRow.driver_id, … }) })` whose body opens with `const { driverId } = ctx.input; if (!driverId) return ctx.skip('no driver on this outcome');`. The job always *runs*, but `ctx.skip()` (§9.4, ADR-035) records the branch-not-taken with a reason so a no-op is distinguishable from real work in Observability and Compare Mode (`condition_not_met`, §14) — not a silent `return`. This is the right home precisely because the absence of a driver is not a different event — it is the same `outcome.resolving` event with less to do.
...
- `detector` matches `ctx.actionName`; the endpoint's existing logic (validation, the SDK query/mutation, side effects) becomes the module's **`jobs`**, and the response is shaped at **`kit.handler({ after: { fromResults } })`**, which reads those job outputs and **returns the output type**. Wrap expected failures in `ActionError(message, code?)` (→ the 4xx `{message, extensions}` Hasura surfaces); the **generic** platform adapter (`netlifyV2Platform` for v2 functions, else `netlifyPlatform`) maps the `fromResults` return→2xx and the error→4xx — there is no dedicated action platform (§7.2, ADR-026/044).
...
- **Resolve the dependent code:** every helper the endpoint imports (payload mappers, dealership mapping, SDK GraphQL ops, validators) carries over, now called from `jobs` (and the `after: { fromResults }` reply fn); the response-shaping code becomes the `after: { fromResults }` return; the `JSON.parse(event.body)` + manual arg validation is replaced by the typed `ctx.input` (Hasura validated it against the input type).
...
## 19.3 Reverse-integration migration hazards [v0.3.20]

The reverse integrations (super-dispatch / central-dispatch / carsarrive) are the sharp edges. Each needs a deliberate call in the per-endpoint checklist, not a mechanical port.

- **Ack semantics flip — choose the `after` mode per endpoint, explicitly.** SD/CD/CA return **500 on business-logic failure today**, which is what drives the vendor to retry. EventKit's fire-and-forget webhook default acks **200 regardless of job outcome**. Porting one of these endpoints without deliberately picking the reply — `after: { body }` for a constant fast ack, or `after: { fromResults }` composed from the run (job-failure → 5xx) — silently turns "the vendor retries our failures" into "we ack and swallow them." The migration checklist MUST record the ack choice for every webhook endpoint. (Validation of the design: super-dispatch's handler is accidentally a *background* function today, so its computed 401/500 responses are already dead code — EventKit's `deferredResponse` guardrail rejects `after: { fromResults }` on a background platform at handler creation, which would have caught that.)
- **`order.created` is the one real fan-out — budget it as a redesign.** Super-dispatch `order.created` splits an order into N sub-orders → N moves + contacts + an optional chase move, plus a deliberate 3-second `setTimeout` race-delay hack. That is dynamic multiplicity, which ADR-025 forbids at the array level. It moves **inside one job** or into **DB-write-triggered follow-on events**, and the `setTimeout` hack is replaced by real idempotency, not carried over. Use it as the test of the "fan-out via DB writes" doctrine.
- **The hoprides ride orchestrator is explicitly OUT OF SCOPE for the first migration.** `batch.created.ride.orchestrator` is a self-rescheduling, long-running polling loop wearing an event-module costume. It strains the current system and would strain EventKit the same way. Decision: it **stays a bespoke background function** for now; do not let it be the first module someone "creatively" migrates. EventKit growing a continuation/reschedule story is a possible future feature, not a launch dependency.
- **Chaining via domain columns is the primary recipe; `event_correlations` is the fallback.** The production chaining pattern already exists: `moves.reference_num` = SD order id / CD dispatch id / CA load id; `ridehails.external_id` = Uber/Lyft ride id. `correlationResolver`'s `extractKey`/`lookup` (ADR-028) supports this directly, so the **domain-column recipe is the primary Mechanism B variant**; the `event_correlations` table is the fallback only for vendors with no domain home. hoprides is the ideal chaining *pilot* — the full app→DB→job→vendor→webhook→DB loop already exists there.

# 20. Testing Strategy

Validate at multiple layers: detector unit tests (insert/update/delete, changed/unchanged columns, missing rows, **manual invocations**, malformed payloads); module tests (assert the declared `jobs` array is the expected, unconditional set — and that a conditional entry is a type/throw error per §3.9 — plus `prepare` where present, and the invocation's `after: { fromResults }` reply fn where declared); job tests (idempotency, retry safety, outputs, external errors, timeout, cancellation, reading `ctx.input` — including a plugin-injected baseline merged with handler input); plugin lifecycle tests (idempotency, failure handling, buffer/flush); source adapter normalization tests (incl. provenance extraction and `HasuraHandlerContext` construction); runtime integration tests through `createEventKit()`/`handle()`; flow tests; golden-trace tests for observability regressions.
...
**[v0.3.21] The suite is the executable form of the spec.** This is a mission-critical, low-churn backbone: many repos will lean on it, and it won't change often, so the bar is "a green build *is* the claim that the package does what the docs say." Every normative MUST in this document SHOULD map to a named test, and `describe` blocks are keyed to their ADR/section. The full plan — the error-path chaos matrix, lifecycle-ordering and golden-trace snapshots, the response-mapping matrix, property tests, and the surface-freeze + docs-compile gates — lives in **`docs/planning/testing-strategy.md`**. The three highest-value additions, called out here because they guard silent failure modes: (1) an **error-path matrix** that injects a throw at every seam (normalize, `configureInvocation`, `augmentEnvelope`, `buildDetectorContext`, detector, `prepare`, `buildHandlerContext`, job body, the `after: { fromResults }` reply fn, every `on*`, `onFlush`, the sink) and asserts the same invariants each time — `ok` is truthful, retry semantics correct (framework 500 vs client status vs isolate-and-continue), `onError` fired, `onInvocationEnd`/`onFlush` always ran, no buffer/listener growth (ADR-033); (2) **golden-trace observability snapshots** with a seeded id generator into a memory sink — the record schema is the contract with Grafana and the Console; (3) a **`formatResponse` matrix** (`{resolved output, ClientError, ActionError, framework error, timeout}` × every platform) since that mapping is what a vendor's retry contract actually sees. A **docs-compile gate** (typecheck the fenced snippets in the RFC/guide/README, or reference compiled `__examples__`) is normative CI: it eliminates the doc-drift defect class (e.g. the stale `observability` registration example).

**[v0.3.21] The published test harness `@hopdrive/eventkit/testing` is a versioned public surface (ADR-036).** Consumers MUST test their modules through the **real runtime**, never through mocks of it (the legacy failure mode was mocks of `run` arity / `job()` shape that drifted from reality). The harness ships in the same package so runtime and harness can never version-skew, and it is held to the same stability + API-snapshot discipline as the runtime. Its contents and the standard consumer test pyramid are specified in `testing-strategy.md` and ADR-036.
...
## ADR-044: The HTTP reply is an invocation-layer declaration — `kit.handler({ before, after })`; modules own only detection + jobs. [2026-07-09]
Decision: the synchronous HTTP reply stops being a per-module seam and becomes a property of the **invocation layer**. A module declares only a detector and a **static `jobs` array** — `jobs` is now REQUIRED in effect (a module with no jobs does nothing → `registerEvent` throws), and a module that declares `resolve`, `respond`, or `response` throws at register time with a pointer to `kit.handler({ after })`. The endpoint's one reply is declared at **`kit.handler({ before, after })`**: **(1)** `before?: (…rawArgs) => HandlerShortCircuit | void` is a **pre-dispatch gate** (auth / HTTP method) that runs on the RAW platform args *before* `normalize`, so it cannot see the parsed envelope or `signatureVerified`; return `{ status, body?, headers? }` to short-circuit before any module runs (the platform's `formatRejection` shapes it to the wire), or `void` to proceed. **(2)** `after?: HandlerResponse` is the reply declaration, applied *after the run settles* — exactly one of two self-naming, mutually-exclusive modes plus optional `status`/`headers` (`ResponseWire`, the web-standard `ResponseInit` fields as data): `{ body }` is a **constant** reply (data, not code, so the work provably cannot wait on or change it — the fast ack, e.g. Stripe's constant 2xx receipt), and `{ fromResults: (result) => body }` runs **arbitrary business logic over the full typed `InvocationResult`** the runtime builds — every detector's verdict as an `EventOutcome { name, detected, jobs, error? }` and every job as a `JobExecution { jobName, status, output, error, … }`, spanning ALL detected events on the request — throwing `ClientError(status, …)`/`ActionError(message, code?)` to produce the error mapping. Request/response sources (`hasuraAction`, status-contract webhooks) do their work as durable **jobs** and compose the reply from those outputs via `after: { fromResults }`, never via a module seam. Platform vs `after`: "202 now, then run the work" is a **platform** choice (`netlifyBackgroundPlatform` / any `deferredResponse` platform), NOT an `after` mode — `{ fromResults }` is rejected (loudly, at handler creation) under a deferred/background platform because its value could never reach a reply that already left, while `{ body }` is allowed but its body is ignored there. Guardrails: `after` is validated at handler **creation** (module scope), is **skipped on a framework error** (a 500 always reaches the wire — the retry contract), and **never clobbers a pre-dispatch client rejection** (a webhook `rejectUnverified` 401 keeps its status).
Rationale: one invocation has exactly one wire reply, so the reply belongs to the layer that owns the endpoint, not to whichever of N detected modules happens to declare it. The per-module `resolve` (ADR-026) and `respond` (ADR-029) seams forced the reply into the (sibling-ignorant, fire-and-forget) module model, made "which module answers?" ambiguous once more than one event fires, and split the fast-ack vs result-driven reply across two authoring shapes. Hoisting the reply to `kit.handler` makes the two real choices self-naming data (`body` vs `fromResults`), gives the pre-dispatch gate (`before`) an honest home on the raw args, and lets `fromResults` compose over the complete, typed run rollup instead of one module's view. `{ body }` being data (not a function) is what lets the runtime prove the reply cannot depend on the work — the property a background/deferred platform needs.
Consequence: re-amends ADR-026 and supersedes ADR-029's `respond`. `EventModule` loses `resolve`/`respond`; a module declaring either (or `response`) throws at register time; `jobs` is effectively required. Removed types: `ResolveFunction`/`RespondFunction`/`FlowResponseKind`/`KitEventDescription.response`, plus the response-kind in `describe()`/flow output and the `expectFlow(...).respondsWith(...)` testing assertion. `kit.handler` gains the typed `{ before, after }` options (`HandlerResponse`/`HandlerShortCircuit`/`ResponseWire`); `InvocationResult.resolved` is now produced by `after` (or a pre-dispatch client rejection), not by a module. The generic-platform wire mapping of `resolved` (output → 2xx, `ClientError`/`ActionError` → status/4xx) is unchanged — only the authoring surface moved.

## ADR-042: Selective replay — a request-scoped job filter, `'skipped'` as the framework's non-execution status, and the `replaySafe` declaration. [v0.3.24]
Decision: an operator can re-POST a recorded invocation with a replay directive in `x-eventkit-*` headers (`x-eventkit-replay-of: <invocation id>`, optional `x-eventkit-replay-jobs: <JSON selection>`), and the runtime executes only the selected jobs; every unselected job of a detected event is finished as **`status: 'skipped'`** (reason `rerun job not selected`) without running — with BOTH job hooks emitted so observability records the row. The pieces: **(1)** core defines `JOB_FILTER_KEY` + `JobFilter` + a pure `jobSelected()` matcher; the filter rides **`request.meta[JOB_FILTER_KEY]`** (request-channel control state; contributed via `configureInvocation`, which sees the request — `augmentEnvelope` does not — and is auto-persisted into `invocations.context_data`). **(2)** `runOne` checks the filter before `onJobStart`; a filtered-out job never invokes its fn (no retries, no reportError). **(3)** A new opt-in plugin `replayControls()` parses/validates the headers (malformed → branded `ClientError(400)` — never a silent full run or full skip; optional `requireToken` → 401) and stamps both the filter and the audit marker `request.meta.replay = { of, only }`. **(4)** `JobOptions.replaySafe?: boolean` is a tri-state declaration (safe / unsafe / undeclared), persisted per execution into `job_executions.job_options` and reported by `kit.describe()`; it is advisory — the Console's warning ladder (none / caution / blocking acknowledgment) is the enforcement point, with runtime hard-skip a later opt-in. **(5)** The status pair is API: `'skipped'` = the framework/operator chose not to run the job; `ctx.skip()` stays `'completed'` + `metadata.conditionNotMet` = the job ran and chose not to act. **(6)** Selection only NARROWS — no directive can run a job dispatch wouldn't have run, alter input, or bypass function auth / webhook verification / suppress-dispatch. Lineage: the Console re-sends the original inbound token header, so a replay lands as the original invocation's sibling (same correlation id), distinguished by `context_data.replay`; omitting the token ("fresh root") is the explicit fallback. Prerequisite: the legacy observability CHECK constraint allows only `{running, completed, failed}` for job status — the sink's default `statusMap` folds `skipped → 'failed'`, so shipping replay requires widening the CHECK (additive DDL) and passing `skipped` through. Full design: `docs/planning/selective-replay-spec.md`.
Rationale: the architecture had every piece waiting — an unclaimed `'skipped'` status both ok-predicates already accept, headers normalized onto `request.meta` by every platform, the original body/headers/rawBody already persisted (`source_event_payload` + `context_data`), and the chain-guard keys as precedent for a core-owned control key that plugins set and the runtime enforces. The alternative (bespoke replay endpoints per repo) re-implements dispatch and can't record honest skip rows.
Consequence: `core/job-filter.ts`; a ~15-line `runOne` gate; `replayControls` plugin; obs persists `execution.metadata` (also closes the ADR-035 gap where `conditionNotMet` reasons were dropped) and stamps `replaySafe`; sink status-map + observability CHECK widening; Console work (relay + modal) specced separately in the console plan's home (`console/`).

## ADR-043: Declarative job-module config — one `JobOptions` interface, writable on the function (`config`) and overridable per call site. [v0.3.24]
Decision: `JobOptions` is THE single settings interface for a job — `name`, `timeoutMs`, `retries`, `tags`, `input`, `metadata`, `replaySafe` (ADR-042) — and it can now be declared **on the job function itself** as a `config` property, Netlify-Functions-v2 style, so everything about a job lives in the job's own module. `defineJob(fn, config)` is the typed sugar (attaches `config`, returns the function). Resolution happens once, inside `job(fn, opts?)` (which the register-time normalization and the runtime's bare-fn auto-wrap both route through): `resolved = { ...fn.config, ...definedProps(opts) }` — per-property shallow, call-site wins, module config applies only where the call site passed nothing (`undefined`-valued call-site keys count as not passed; no deep merge of `metadata`/`input`). Name resolution becomes `opts.name || fn.config.name || fn.name || 'anonymous'`. The ADR-031 banned-key guard (`continueOnFailure`, `run.mode`) applies to the merged bag, so module config cannot smuggle rejected options. `kit.describe()` reflects the merged (effective) options.
Rationale: the split today forces job settings to the registration site, away from the code they describe — a job's timeout, retry posture, and rerun-safety are facts about the job, not about the event wiring it into a module. Keeping ONE interface (rather than a parallel "module config" type) means no drift between the two write sites, and the precedence rule is the one every config system users already know (specific beats general). Attaching config to the function (not a separate module export) is forced by the registration contract: eventkit receives function references, never file paths — the config must travel with the function object.
Consequence: `JobOptions` gains `replaySafe`; new `defineJob` + `ConfiguredJobFunction` type exported from core; `job()` gains the merge + undefined-stripping; register guard extended to merged options; `describe()` gains `replaySafe`; guide "Controlling how the jobs run" section documents the two write sites and the precedence rule.

## ADR-039: Chain-concern placement — codec in core, extraction in sources, policy in generic plugins, injection in the write layer. [v0.3.23]
Decision: the chaining machinery is re-homed along the existing layering rules (ADR-024: sources own their payload shapes; plugins are generic-by-config; core owns seams). **(1) Codec → core.** The tracking-token codec (`create`/`parse`/`isValid`/`withJobExecutionId`/hop-depth) moves to `core/tracking-token.ts` — it is a pure, dependency-free primitive consumed by `loopGuard`, `correlationResolver`, the sources, and (eventually) sdk-* helpers; today `correlationResolver` imports it from inside `plugins/loop-guard/`, which is the tell. **(2) Inbound discovery → sources.** A source may surface ordered token candidates on the envelope during normalize: `meta.tokenCandidates: string[]`. The Hasura family (event/action/cron via `hasura-shared`) reads its two channels — the row write field (`updated_by`/`updatedby`, configurable) then session variables (default list includes `x-hasura-tracking-token`; Hasura forwards any `x-hasura-*` request header on admin-secret requests into `event.session_variables`, which is the zero-persistence header channel the E2E proof validated). The webhook source contributes no candidates (vendor bodies are unknowable — `correlationResolver`, ADR-028, is its recovery path); the application/fake source contributes none. **(3) `loopGuard` sheds all payload anatomy.** Its Hasura-shaped defaults (`event.data.new`, `event.session_variables`) and per-shape config (`getRow`/`getSession`/`read`/`extractFrom*`/`updatedByPattern`/`sessionVariables`/`metadataKeys`) are removed (pre-1.0, no compat shim); it consumes `meta.tokenCandidates` (plus a single generic `candidates: (envelope) => string[]` escape hatch), lifts lineage (correlation id, `sourceJobId`, hop depth) from the first candidate that parses, continues/mints `ctx.trackingToken` (ambient, unchanged), and owns the hop ceiling. **(4) Outbound injection is a write-layer concern.** EventKit provides the token (`ctx.trackingToken`); attaching it belongs to whoever owns the write. The eventkit-native paved road uses the **existing ADR-020 input-baseline seam** (`augmentJobContext → { input }`): a Hasura companion helper (`hasuraChainedClient({ endpoint, headers })`, shipped with the Hasura source package) contributes `ctx.input.gql`, a fetch wrapper that attaches `x-hasura-tracking-token: ctx.trackingToken` to every request. The durable org-wide answer remains the sdk-* write family injecting the header itself. **(5) Observability stays a lineage consumer** — it reads `meta.sourceJobId`/correlation and never parses payloads or injects headers. **(6) The HopDrive preset (`hopdriveLoopGuard`, referenced by §13 but never built) ships now**, pinning separator `'|'`, write field `updated_by`, the `x-hasura-tracking-token` session variable, `validateCorrelationId: true` (safe once ADR-040 lands), and required `serviceId`.
Rationale: the placement audit after the live E2E proof found four deviations from the layering line: a shared primitive homed inside one plugin; a "generic" plugin shipping Hasura payload anatomy (deepened when session-variable parsing was added to it); header injection with no home at all (the proof hand-rolled `gqlWithToken` in job code — the seam existed, ADR-020, unused); and a preset promised by the RFC that doesn't exist. Fixing placement now is cheap; after rollout every consumer config would break.
Consequence: codec file move + import updates; `hasura-shared` gains the candidate reader + config; `loopGuard` config surface shrinks (breaking, pre-1.0); new `hasuraChainedClient`; `hopdriveLoopGuard` preset; `docs/chained-events.md` config snippets updated. `correlationResolver` is untouched (already generic).

## ADR-040: A correlation id is UUID **or** 32-hex dashless; Hasura trace-id adoption stays. [v0.3.23]
Decision: the codec's `validateCorrelationId` accepts two shapes — canonical UUID (8-4-4-4-12) and 128-bit dashless hex (32 hex chars). The Hasura source keeps adopting `trace_context.trace_id` as the root correlation id (precedence unchanged: explicit request → trace id → generated).
Rationale: the E2E proof hit this live: Hasura trace ids are 32 hex **without dashes**, so UUID-only validation rejected every token built from a trace-rooted chain — silently. Extraction fell back to treating the whole token as a bare id, tokens nested inside tokens, `sourceJobId`/hop depth were lost, and nothing errored. Alternatives rejected: minting a UUID instead of adopting the trace id (loses the free APM linkage between chains and traces); shipping the preset with `validateCorrelationId: false` (loses the guard against arbitrary garbage occupying the correlation slot).
Consequence: one widened regex in the (now-core) codec + tests for both shapes; the HopDrive preset can keep validation on; the loop-guard-local UUID copy consolidates into the codec with the ADR-039 move.

## ADR-041: A halted chain is a first-class, loud event — branded error, durable marker, warn escalation. [v0.3.23]
Decision: when the hop ceiling (`haltAtDepth`, ADR-034) suppresses dispatch: **(1)** the runtime reports a **branded `LoopDetectedError`** (name pinned; fields: correlation id, depth, ceiling, `serviceId`, source function/table) through `reportError` with phase `'chain-guard'` — `onError` subscribers route on the brand (the grafana plugin picks it up today; a Sentry plugin is a trivial `onError` subscriber); **(2)** the invocation record carries a **durable halted marker** — `error_message` = the halt reason and `context_data.halted = { depth, ceiling }` — so halted chains are queryable in observability instead of reading as benign zero-event successes; **(3)** `warnAtDepth` ALSO reports the branded error as **non-fatal** through `onError` (severity warn), not just a log line — the early alarm fires while the chain is still running; **(4)** the HTTP response stays **200** — never ask a source to retry a loop (a retry would re-enter at the same depth and halt again, but invites vendor-side alarm fatigue and duplicate suppression work).
Rationale: stopping a runaway chain is not the same as *knowing about it*. Today's halt path already stops the loop and reports through `onError` + logs + flushes the invocation (kit.ts suppress seam) — but the error is an unbranded `Error` (Sentry can't alert-route it), and the durable record is indistinguishable from a successful no-op invocation (nothing queryable says "chain killed here"). The operational requirement is: halt in core, elevate through the plugin hooks so any alerting backend picks it up without bespoke wiring.
Consequence: `LoopDetectedError` in `core/errors.ts` (serializable, brand-checked like `ClientError`); the kit.ts suppress path upgrades its `reportError` call and enriches the invocation record; `loopGuard`'s warn path gains the non-fatal `onError` report; chaos-matrix rows pin halted-invocation observability. No wire-status change.

## ADR-038: A source declares its crash policy — a processing crash's retry contract is per-transport. [v0.3.22]
Decision: `EventKitPlugin` gains an optional `crashPolicy: 'ack' | 'signalRetry'`, declared by the **source** and read off the resolved source (like `sourceType`). Framework default is `'ack'`: an unhandled **detector** or **`prepare`** crash stays in `events[].error`, the invocation still returns **200** (no top-level `result.error`), and an at-least-once transport does not retry. `'signalRetry'` escalates that same processing crash to a top-level `result.error` → **500** → the sender retries, and emits a prominent `invocation.log.error` (Grafana visibility) on top of the `onError` route (Sentry) that `reportError` already fired during detect/dispatch. A deliberate `resolve`/`respond` reply is **not** a crash — it still maps to its client status and is never escalated; a job failure is not a processing crash either — it keeps its own durable retry and stays a 200. The **webhook source defaults to `'signalRetry'`** (overridable via `webhook({ crashPolicy })`); Hasura and application sources stay `'ack'`.
Rationale: the error-path chaos matrix (ADR-036) surfaced that a detector/`prepare` crash acks 200 by design — correct for a Hasura event trigger (a poison row retried forever is worse than a dropped one, the legacy hasura-event-detector contract) but wrong for an inbound vendor webhook, where the crash silently drops an event the vendor would have redelivered. A single framework default can't serve both transports, and a crash this severe on the reverse-integration money path must be *loud* (Grafana + Sentry) as well as retryable. Making it a source-declared property keeps the choice with the plugin that knows its delivery guarantees, and preserves 100% back-compat (default unchanged for existing sources).
Consequence: `EventKitPlugin.crashPolicy` + a `PluginManager` getter + a `kit.ts` result-assembly escalation with a loud log; `WebhookConfig` gains `crashPolicy`. The error-path matrix now pins both policies. No consumer change required; webhook consumers get 500-and-retry automatically.

## ADR-036: The published test harness (`@hopdrive/eventkit/testing`) is a first-class, versioned public surface. [v0.3.21]
Decision: consumers test their modules through the **real runtime**, never through mocks of it, and the helpers that make that ergonomic ship from `@hopdrive/eventkit/testing` — same package, same version, same stability + API-snapshot discipline as the runtime. The harness provides: per-source **payload builders** (Hasura insert/update/manual/cron/action, signed webhook requests); a **recording harness** (`testInvocation(kit, payload)` / `recordingPlugin()`) that runs `handle()` and returns which detectors fired, each job's resolved input/metadata, executions, logs, `resolve`/`respond` output, and the hook sequence; a **`detectorContract(module, { fires, suppresses })`** helper that auto-appends a MANUAL-operation case for `hasuraEvent` modules; **memory doubles** (`memorySink`/`memoryBatchStore`/`capturedLogger`); a **`simulateChain()`** correlation harness (Mechanism A `echoWebhook` / Mechanism B `lookupWebhook` + a miss → clean-root case); and **flow assertions**. The guide documents one standard consumer test pyramid (many detector-contract tests → one flow snapshot per kit → a few full-kit integration tests → `eventkit-flow check` in CI) so every repo looks alike.
Rationale: the legacy suite's failure mode was mocks of `run` arity and `job()` shape that drifted from the real runtime (§20). Shipping the harness in-package removes version skew and makes the mock-free idiom the path of least resistance. The `detectorContract` auto-MANUAL case mechanizes the one named silent-regression risk (console-edit suppression, D17) into every repo by default. `simulateChain` serves the chaining strategy (ADR-028) directly — a repo asserts "the vendor webhook rejoined the chain" in CI with no vendor involved.
Consequence: `./testing` grows from its current four helpers; it gets its own API-snapshot test; breaking it is a major-version event. Built to the `testing-strategy.md` spec.

## ADR-037: The flow artifact carries topology metadata now; the cross-kit aggregator is deferred. [v0.3.21]
Decision: extend the shipped flow generator (ADR-032) along its trustworthy axis (`describe()` can't lie about a declarative module's job set). Add `toFlowMermaid(kit)` (diff-readable in PRs), an `eventkit-flow coverage` subcommand that fails on any event lacking a detector-contract test, and an `eventkit-flow simulate --payload fixture.json` that runs the (near-pure) detectors against a fixture and prints which events/jobs would fire. Reserve, in the manifest schema **now**, `repo`/`function` fields on the graph and a job **`metadata.effects`** convention (`{ type:'db-write', table } | { type:'api-call', vendor }`) — cheap forward-compat so a future org-level aggregator can merge every repo's committed YAML and *infer* cross-kit edges (job writes `moves` → the kit listening on `moves`) with confidence, for hand-authored manifests to confirm.
Rationale: the generator's admitted blind spot is cross-event chains and business intent (§15). Most of the cross-kit gap closes with a convention, not runtime work — but only if the schema fields exist before the repos generate their docs, so aggregation is lossless later. The aggregator itself, and Compare Mode, stay phased (D9). A proto-Compare CI helper (observed record set ⊆ expected graph, via the shared node-id builders in `console-expected-flows.md` §4) validates the matcher hypothesis on one flow in a unit test, months before the Console.
Consequence: `./flow` gains emitters + two CLI subcommands; the YAML schema gains `repo`/`function`/`effects`; the org-wide topology becomes a machine-derivable artifact on the roadmap without blocking on the Console.

## ADR-033: Pre-dispatch pipeline hardening — isolate transforms, brand-check rejection, always flush. [v0.3.20]
Decision: the pre-dispatch delta-transform hooks (`configureInvocation`, `augmentEnvelope`) run **isolated** — each in its own try/catch, a throw routed to `onError` and the pipeline continuing with the last-good merged value (best-effort, same philosophy as observability write failures, §11.3). The pre-dispatch **`ClientError` fast-path is brand-checked**, not bare `.status` duck-typed: only a genuine, intentional `ClientError` (a branded marker, e.g. `isClientError(err)`) maps to a client response via `resolved.error`; any *other* thrown error becomes the framework **500** so the vendor retries — never a silent `{ ok: true }`. `onInvocationEnd`/`onFlush` move into a **`finally`** so the invocation record always flushes and the buffer never leaks on a pre-dispatch throw. And `augmentEnvelope` deltas are **deep-merged at `meta`** (`meta: { ...merged.meta, ...partial.meta }`), so a plugin returning `{ meta: { myKey } }` can't wipe another's `sourceTrackingToken`/`sourceJobId`.
Rationale: today a throw from any pre-dispatch hook lands in the outer catch, which duck-types `.status` on *any* error and returns `{ ok: true, resolved: { error } }` — detection skipped, jobs skipped, no top-level error, no vendor retry. The worst case is real: `correlationResolver` runs an app-injected DB lookup in `augmentEnvelope` (ADR-028) with no internal catch, so a transient DB blip whose error carries a numeric `.status` **silently drops an entire inbound webhook and reports success** on the exact chain-reconnection seam the reverse integrations ride. The shallow meta-merge is latent — safe only because `loopGuard`/`correlationResolver` defensively rebuild `meta` today; the first plugin that doesn't reopens the event-storm hole ADR-016 closes.
Consequence: fixes P0-1/P0-2/P0-3. A load-bearing transform that must fail the request loudly (rather than degrade) opts in explicitly (a `strict` flag or an intentional `ClientError` with a 5xx status), not by accidental throw. New tests MUST cover a throwing `configureInvocation`/`augmentEnvelope` (isolation + flush + no false `ok`) — there are zero today.

## ADR-034: loopGuard gains an optional hop-depth ceiling. [v0.3.20]
Decision: `loopGuard` may carry a **hop counter** in the token/`meta` that increments each hop, with config `warnAtDepth` (log + `onError` breadcrumb) and `haltAtDepth` (hard-stop: suppress dispatch and log). Off by default (depth unbounded = today's behavior).
Rationale: loop prevention today is loop *traceability* — the token makes a self-triggered write recognizable but nothing bounds a cycle; an A→B→A loop where one detector forgets its guard runs forever and the token only shows it after the fact. Event storms are the stated catastrophic failure mode (ADR-016). A generic, config-driven ceiling converts "hope every detector remembered the guard" into a bounded blast radius. Fits ADR-024 (generic-by-config).
Consequence: net-new capability, not parity. Opt-in per repo via the shared preset; suppression stays a per-detector decision unless the ceiling is set.

## ADR-035: `ctx.skip(reason)` — a first-class job self-skip that records `condition_not_met`. [v0.3.20]
Decision: `JobContext` gains `skip(reason: string): never`. Calling it stops the job (throws a runtime-recognized `SkipSignal` the executor catches) and records the outcome as a branch-not-taken: structured `metadata.conditionNotMet = { reason }` on the `JobExecution`, terminal status stays `'completed'` (the job ran; it chose to do nothing). It does **not** use the reserved `'skipped'` status (that stays for the deferred series short-circuit, ADR-031).
Rationale: the Pattern-B short-circuit (`if (!x) return;`, §19.1) completes normally, so its record is identical to a job that did real work — hiding the branch every time a condition isn't met. The consumer audit found ~21 such sites, and the hard ones gate on data fetched mid-handler (async), so they can't move to the detector (§8) and must live in Pattern B. Compare Mode already lists a `condition_not_met` classification (§14) with nothing producing its data. `ctx.skip()` is that producer.
Consequence: the ~21 conditional sites migrate without losing branch visibility. Observability/Compare render a no-op distinctly. A small API addition now versus a migration-wide retrofit later.
...
**[v0.3.18, ADR-031, amends ADR-014] Series is deferred. Parallel is the only mode at launch.** We built the `run.mode` switch (`'series'`) and its `continueOnFailure` companion (module- and per-job), but they're not enabled. The public `RunOptions`/`JobOptions` leave them out, and the runtime runs every module's jobs in parallel with isolated failures. Why: series lets you order one job before another, or feed one into the next. That's the sequential, handler-style coupling ADR-025 removed, and it's more abuse risk than value right now. No consumer needs it yet. It's still a possible future feature. If a real, reviewed ordering need shows up, it comes back behind this same `run.mode` API, so no existing module changes. ADR-014's parallel+isolated behavior doesn't change here; ADR-031 just takes away the opt-in for now.
...
**[2026-07-09 — RE-AMENDED by ADR-044; supersedes the `resolve`/`respond` framing above] The reply moved to the invocation layer.** The per-module `resolve` (this ADR) and `respond` (ADR-029) seams are **removed**. A module owns only detection + a static `jobs` set; the endpoint's one wire reply is declared at **`kit.handler({ before, after })`** — `after: { body }` (a constant ack, data not code) or `after: { fromResults: (result) => body }` (composed over the full `InvocationResult` rollup, after the run settles). Request/response sources (`hasuraAction`, status-contract webhooks) do their work as durable jobs and compose the reply from that rollup rather than from a separate module seam. The response contract (output → 2xx, `ClientError`/`ActionError` → status/4xx `{message,extensions}`) still composes onto the generic platforms via `InvocationResult.resolved` exactly as above; only the *authoring surface* changed. Removed types: `ResolveFunction`/`RespondFunction`/`FlowResponseKind`/`KitEventDescription.response`, the response-kind in `describe()`/flow output, and the `expectFlow(...).respondsWith(...)` testing assertion. See ADR-044.
