...
**[v0.3.19 — ADR-032] The Expected-Flow *generator* is now SHIPPED; Compare Mode is still phased.** The structural half of this triad exists in the package today: `kit.describe()` + `@hopdrive/eventkit/flow` (`toFlowGraph`/`toFlowYaml`) derive the Expected graph directly from the declarative modules, and the Observability records already are the Observed evidence. What remains phased is the **matcher/Compare** join and the Console UI over it — planned in `console-expected-flows.md`, still to be proven on one flow. The generator emits stable, path-independent node ids (`source`, `event:<name>`, `job:<event>:<job>`, `sideEffect:<job>:<effect>`) exactly so the eventual overlay lines up.
...
**[v0.3.19 — ADR-032] The generator side is implemented; manifests stay hand-authored.** The `FlowNode`/`FlowEdge`/`FlowManifest` types now ship from `@hopdrive/eventkit` (core), and `@hopdrive/eventkit/flow`'s `toFlowGraph(kit)` emits the structural `{ nodes, edges }` skeleton in exactly this vocabulary — so a generated graph can be diffed against, or promoted into, a hand-authored manifest. `kit.describe()` is the underlying pure registry walk; `toFlowYaml(kit)` is the committed human-readable form; the `eventkit-flow` CLI (`generate`/`check`) produces and CI-gates the artifact from a consumer repo. What the generator *cannot* infer — cross-event chains (a job's DB write triggering another kit's event) and business intent — is what the hand-authored manifest still owns. Manifest **CI validation** (the schema/reference/generated-graph checks above) is the remaining phased piece.
...
**[v0.3.19] Console plan captured.** Now that the Expected graph is a shipped, committed artifact (ADR-032) and the Observed records already exist, the remaining work — the three Console modes, the matcher/Compare join, the backend read API, and the shared node-id contract that makes the overlay line up — is planned in **`console-expected-flows.md`**. The Console itself stays phased (D9): prove Compare on one flow before generalizing.
...
Decision: EventKit ships the **structural** half of the Expected/Observed/Compare triad (§14–§16) as first-class package surface, and holds the meaning/Compare halves for later. `kit.describe()` returns a pure `KitDescription` (source, platform, plugins, and every registered event with its **static** job set) via a read-only registry walk that executes no detector/prepare/job/hook. A new `@hopdrive/eventkit/flow` subpath turns that snapshot into artifacts: `toFlowYaml(kit)` (a committed, diff-friendly YAML document of how events flow through the system), `toFlowGraph(kit)` (`{ nodes, edges }` in the `FlowNode`/`FlowEdge` vocabulary of §15), and `describeKit(kit)`. The `eventkit-flow` bin (`generate` / `check`) lets a consumer regenerate the doc in an npm script and gate drift in CI. The core exports the `KitDescription`/`FlowManifest`/`FlowNode`/`FlowEdge` types (this also retires prior doc-drift where §16 listed `FlowManifest` as exported though it wasn't).

Rationale: this is exactly the "generator verifies structure" principle (§15) made real, and it is only *possible* because modules are declarative (ADR-025) — a static `jobs` array with no conditional inclusion or fan-out means the whole structure is knowable **without running anything**, so the generated graph can't lie about what a handler "might" do. It is deliberately scoped to what the package can know from one kit: source → event → jobs (+ side effects declared in job `metadata`). What it cannot infer — cross-event chains (a job's DB write triggering another kit's event) and business intent — stays with the hand-authored Flow Manifest (§15), and the matcher/Compare join + Console UI stay phased (D9; planned in `console-expected-flows.md`). Node ids are derived from event/job names, never file paths (§14), so the eventual Observed↔Expected overlay lines up. Zero new runtime dependencies: the YAML emitter is built in.

Consequence: `EventKit` gains `describe()`; the package gains the `./flow` subpath and the `eventkit-flow` bin; `FlowNode`/`FlowEdge`/`FlowManifest`/`KitDescription` become public types. D9's structural half is now **resolved (shipped)**; its manifest-CI, Compare, and Console-backend halves remain open/phased.
