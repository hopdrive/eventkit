...
- **Code org → a FLAT `src/plugins/`**: every plugin its own folder **directly** under `plugins/` with **no subcategory folders** (no `sources/`/`platforms/`/`transports/`); the folder name implies the kind (`hasura`, `webhook`, `lambda-platform`, `netlify-platform`, `netlify-v2-platform`, `netlify-background-platform`, `observability`, `batchjobs`, `loop-prevention`, `grafana`, `sentry`; plus support files `platform-shared.ts` + `platforms.ts` barrel). **Public subpaths stay short & unchanged** (`./sources/hasura`, `./platforms`, `./plugins/observability`, `./plugins/transports/grafana`) — the `exports` map decouples public path from internal folder. *(Rationale for flat over categorized: the kind is already carried by `provides` + the name, so subcategory folders were redundant.)*
