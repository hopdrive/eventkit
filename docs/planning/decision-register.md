...
- **Status:** ✅ **implemented and verified.** typecheck clean; 5 new runtime tests (sequencing-after-jobs, failed-job to error response, mutual-exclusion, requires-jobs, background-platform rejection); full suite 117 green. **[Superseded 2026-07-09 by ADR-044 — the reply moved to the invocation layer (`kit.handler({ before, after })`); `respond` removed. See D42.]**
...
### D37. Chain-concern placement — who owns token generation, parsing, discovery, and injection [added 2026-07-02]
**Question:** The live chaining E2E proof (session-variable header channel + vendor round trip; `docs/chained-events.md`) worked, but the placement audit behind it found layering leaks: the token codec lives inside `plugins/loop-guard/` yet is imported by `correlationResolver`; the "generic" `loopGuard` ships Hasura payload anatomy (`event.data.new`, `event.session_variables`) as its *defaults*, deepened when session-variable token parsing was added to it; outbound header injection has no home at all (the proof hand-rolled a `gqlWithToken` helper in job code); and the `hopdriveLoopGuard` preset the RFC references was never built. Where does each concern belong?
**Origin:** Rob's audit request after questioning whether header detection belongs in a Hasura-specific package, whether it all belongs in observability, and where the gql-injection interface is defined in the plugin arch.
**Decision:** ✅ **RESOLVED 2026-07-02 (ADR-039).** Core owns seams and the codec (`core/tracking-token.ts`); **sources own their payload shapes** — the Hasura family surfaces ordered `meta.tokenCandidates` (write field, then session variables incl. `x-hasura-tracking-token`) during normalize, the webhook source surfaces none (`correlationResolver` is its recovery path); **generic plugins own cross-source policy** — `loopGuard` consumes candidates, lifts lineage, continues/mints `ctx.trackingToken`, owns the hop ceiling, and loses every Hasura-shaped config knob (breaking, pre-1.0); **the write layer owns injection** — eventkit provides the token, and the paved road is `ctx.input.gql` contributed through the *existing* ADR-020 input-baseline seam by a Hasura companion helper (`hasuraChainedClient`), with sdk-* header injection as the durable org-wide answer; **observability stays a pure lineage consumer**; the **`hopdriveLoopGuard` preset ships** pinning `'|'`, `updated_by`, the session-variable name, validation on (per D38), required `serviceId`. NOT a new standalone hasura package — parsing is inbound source knowledge (belongs with the source), injection is outbound transport (belongs with the write layer); bundling them would couple opposite directions.
**Blast radius:** codec move + imports; `hasura-shared` candidate reader; `loopGuard` config shrink; new `hasuraChainedClient`; preset module; `docs/chained-events.md` snippet updates; loop-guard/correlation-resolver tests.

### D38. Correlation-id shape — trace-id roots vs UUID validation [added 2026-07-02]
**Question:** The Hasura source adopts `trace_context.trace_id` (32 hex, no dashes) as the root correlation id, but the codec's `validateCorrelationId` accepts only canonical UUIDs — so every token built from a trace-rooted chain failed parsing *silently* in the E2E proof (tokens nested inside tokens, lineage lost, zero errors). Which side bends?
**Origin:** first chain run of the POC (2026-07-02); flagged as the blocker for shipping the HopDrive preset with validation on.
**Decision:** ✅ **RESOLVED 2026-07-02 (ADR-040).** The codec bends: `validateCorrelationId` accepts UUID **or** 128-bit dashless hex. Trace-id adoption stays (free APM linkage). Rejected: minting UUIDs instead (loses trace linkage); shipping the preset with validation off (loses the garbage guard).
**Blast radius:** one regex in the (now-core) codec + shape tests; unblocks D37's preset.

### D39. Loop-halt elevation — stopping a runaway chain must also raise it [added 2026-07-02]
**Question:** `haltAtDepth` (ADR-034) stops a runaway chain, but Rob's operational requirement is to *know*: today the halt path logs a warning, reports an **unbranded** `Error` through `onError`, and leaves an invocation record indistinguishable from a benign zero-event success. A Sentry plugin couldn't alert-route it; nothing durable is queryable.
...
### D42. HTTP reply moves to the invocation layer — kit.handler({ before, after }) [added 2026-07-09]
**Question:** `resolve` (ADR-026) and `respond` (ADR-029) both live on the event *module*, so the synchronous HTTP reply is a per-module concern. But an endpoint is one invocation over a set of sibling modules, and **any number of them may detect** on a single request. Modules are sibling-ignorant by design (ADR-025) — none of them can see how many others fired, so none can legitimately own the endpoint's ONE wire reply. Worse, a `respond` seam can only read *its own* module's jobs; it structurally cannot compose an answer over the results of *every* detected event. Where does the single reply actually belong, and how do we let it read the whole run?
**Origin:** Follow-through on the ADR-026/029 request/response line. Sharpening "one module owns the reply" against the reality that detection is many-to-one revealed the seam was on the wrong object: the reply is an endpoint property, not a module property. Ground types: `src/core/kit.ts`.
**Decision:** ✅ **RESOLVED 2026-07-09 (ADR-044, re-amends ADR-026, supersedes ADR-029's `respond`).**
- **The reply leaves the module entirely.** Modules own **only** a detector + a **static** `jobs` array. `jobs` is now **REQUIRED**, and a module still declaring `resolve`/`respond`/`response` **THROWS at register** (fail-loud, not silently ignored). The reason is structural: one invocation → one wire reply, and because sibling modules can't see each other and any number may detect, no module can own that single reply. It belongs where the run is singular — the endpoint.
- **The endpoint declares its ONE reply at `kit.handler({ before, after })`.** Two seams, both optional, both validated **at handler creation** (not first request):
  - **`before?: (…rawArgs) => HandlerShortCircuit | void`** — a pre-dispatch gate (auth/method) over the **RAW** args, before `normalize`. Return `{ status, body?, headers? }` to short-circuit the whole invocation; return `void` to proceed. This is the raw-args chokepoint the detector guards and `rejectUnverified` (D27) already needed a home for.
  - **`after?`** — applied after the run settles, as **DATA not code**: exactly one of two mutually-exclusive modes, plus optional `status`/`headers` (the pair is `ResponseWire` — web-standard `ResponseInit` carried as data):
    - **`{ body }`** — a CONSTANT reply. The fast ack, expressed as data: no closure, nothing to read, nothing to get wrong.
    - **`{ fromResults: (result) => body }`** — business logic over the **FULL typed `InvocationResult`**: every `EventOutcome { name, detected, jobs, error? }` and every `JobExecution { jobName, status, output, error, … }`, **across ALL detected events**. This is precisely what the old per-module `respond` seam could not see — it composes the answer over the whole run. Throw `ClientError(status,…)` / `ActionError(message, code?)` to drive the error mapping.
- **"202 now, then work" is a PLATFORM choice, not an `after` mode.** Deferred/background acknowledgement is `netlifyBackgroundPlatform` (`deferredResponse`), where the response is already gone by the time jobs run. Under any `deferredResponse` platform `{ fromResults }` is **rejected at handler creation** (it would read results nobody can still reply with); `{ body }` is fine. Request/response sources (webhook, hasuraAction) therefore do their real work as **durable jobs** and let `after: { fromResults }` compose the reply from job outputs — the durability and retry story stays on the jobs, not on the reply.
- **Precedence guardrails.** `after` is **skipped on a framework 500** (the retry contract of D36/ADR-038 wins — a crashed run must not be papered over by a composed 200), and `after` **never clobbers a pre-dispatch client rejection** (a `before` short-circuit, or the webhook `rejectUnverified` 401 of D27/ADR-030, is already the reply).
- **Removed.** `ResolveFunction` / `RespondFunction` / `FlowResponseKind` / `KitEventDescription.response`; the response-kind field in `describe()` and flow output; and the `expectFlow(...).respondsWith(...)` test assertion. Nothing describes a module reply anymore because modules no longer reply.
**Blast radius:** `EventModule` (drops `resolve`/`respond`/`response`, `jobs` required) + the register/validate guard that throws on their presence; `kit.ts` handler signature (`before`/`after`, `HandlerShortCircuit`, `ResponseWire`) + the handler-creation validator (mutual-exclusivity, `deferredResponse`×`fromResults` rejection) + the settle-time reply assembly (framework-500 skip, pre-dispatch-rejection precedence); core types (delete `ResolveFunction`/`RespondFunction`/`FlowResponseKind`, `KitEventDescription.response`); `describe()`/`./flow` output (drop response-kind); `./testing` (`respondsWith` removed); and the guide (webhook + hasuraAction sections rewritten around `after`, ADR-026/029 marked superseded).
