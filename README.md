...
The CLI just introspects the kit you export from a module. No handler runs:
...
Point `--kit` at a module that exports an `EventKit` (like `export const kit =
createEventKit(...)`), and pass `--export <name>` if it isn't the default or `kit` export.

> This is the structure half of §14 to §16. The generator verifies structure, that's all
> it does. The meaning half (hand-authored Flow Manifests and Compare Mode) is still
> phased. `toFlowGraph` emits the same `FlowNode`/`FlowEdge` shape on purpose, so a
> generated graph can later be diffed against a manifest, or promoted into one.
